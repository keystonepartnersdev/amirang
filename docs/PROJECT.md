...
1. **암 전문의(본점 원장)의 지식을 시스템화**해서, 일반의(브랜치 원장)도 전문의 수준의 치료 판단을 내릴 수 있게 지원한다.
2. **환자 접수 → 영업 상담 → AI 분석 → 원장 컨펌 → 간호 치료**의 전체 플로우를 디지털화한다.

### 1.2 비즈니스 구조

```
본사 (강남 본점)
├── 본점 원장 (종양 전문의) — AI 컨펌 최종 권한
├── 영업팀
└── 간호팀

브랜치 (최대 10개 목표)
├── 브랜치 원장 (일반의 가능) — 1차 판단, 불확실 시 본점에 에스컬레이션
├── 영업담당 (브랜치별 1명 이상)
└── 간호팀
```
...
> AI 에이전트는 의사(원장)의 판단을 **보조**하는 역할만 한다.
> 모든 최종 결정은 반드시 원장이 확인하고 컨펌해야 한다.
...
| `director` | 원장 (본점 또는 브랜치) | AI 리포트 검토, 프로그램 선택, 최종 컨펌, 간호팀 지시 |
| `nurse` | 간호사 (브랜치별) | 치료 스케줄 확인, 세션 완료 기록, 특이사항 입력 |
| `admin` | 본사 관리자 | 브랜치 관리, 치료 프로그램 배포, RAG/AI 설정, 전체 통계 |

### 2.1 브랜치 원장 에스컬레이션 로직

```
브랜치 원장이 AI 리포트를 보고 판단:
  ├── 확신 있음 → 직접 컨펌
  └── 불확실함 → 본점 원장에게 에스컬레이션 요청
                  → 본점 원장 컨펌 → 브랜치로 결과 내려옴
```
...
type PatientStatus =
  ...
  | 'ai_done'      // AI 분석 완료, 원장 검토 대기
  | 'confirmed'    // 원장 컨펌 완료, 간호팀 전달됨
  | 'in_treatment' // 치료 진행중
  | 'completed'    // 치료 완료
  | 'on_hold'      // 보류
```
...
│   │   │   ├── reporter.py     # 원장용 리포트 생성
│   │   │   └── logic.py        # 원장 판단 로직 적용
│   │   └── prompts/
│   │       ├── system.py       # 시스템 프롬프트
...
model Treatment {
  ...
  patient        Patient   @relation(fields: [patientId], references: [id])
  directorId     String    // 컨펌한 원장 userId
  programs       String[]  // 선택된 프로그램 ID 목록
  directorNote   String?   // 원장 지시사항
...
3. 프로그램 매칭 룰
   - 원장이 정의한 판단 로직 (암종 × 병기 × 증상 → 프로그램 추천)
```
...
interface AIReport {
  ...
  warnings: string[];                 // 주의사항 (원장 검토 필요 항목)
...
# 컨펌
POST   /api/treatments                # 원장 컨펌 { patientId, programs, note }
...
- [x] 원장 대시보드 (AI 분석 애니메이션, 리포트, 프로그램 선택, 컨펌)
...
### AI 에이전트 제약
- AI 리포트에는 반드시 "이 분석은 원장의 최종 판단을 보조하는 참고 자료입니다" 문구 포함
- AI가 생성한 내용을 그대로 환자에게 노출하지 않는다
- 원장이 컨펌하지 않은 AI 리포트는 영업/간호사에게 노출되지 않는다

### 브랜치 데이터 격리
- 모든 쿼리에 `branchId` 필터 적용 (RLS 또는 미들웨어)
- 브랜치 간 환자 데이터 공유 불가 (본사 관리자, 본점 원장 제외)
